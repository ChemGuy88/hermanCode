...
$(basename "$0") -h
$(basename "$0") [-btv] -p pid[,pid[...]] [-f snapshot_frequency] [-o out_path]
...
IFS=',' read -ra pid_array <<< "$pid_string"
unset IFS
...
if [ "$bg_mode" = "TRUE" ]; then
    export -f monitoring_loop
    nohup bash -c "monitoring_loop '$command_string' '$snapshot_frequency'" &> "$out_path" &
else
